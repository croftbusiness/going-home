  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/executor/callback`,
        },
      });
        ...
        <div className="bg-[#FCFAF7] rounded-lg shadow-sm p-8 border border-gray-200">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-[#2C2A29] opacity-70">
              Sign in with Google to access accounts you're executor for
            </p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <p className="text-xs text-[#2C2A29] opacity-60 text-center">
              You must have received an invitation email to access an account. If you haven't received one, please contact the account owner.
            </p>
          </div>
        </div>
      </div>
