...
export default function ViewerFinalSummaryPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!parsedSession.permissions.canViewFinalSummary) {
        return;
      }

      const response = await fetch('/api/viewer/data?section=finalSummary');
      if (!response.ok) {
  ...
  return (
    <PermissionGate permission={session.permissions.canViewFinalSummary || false} sectionName="Final Summary">
        ...
        <header className="bg-[#FCFAF7] border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-semibold text-[#2C2A29]">Final Summary</h1>
          ...
          {data ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
              <div className="flex items-center space-x-3 mb-6 pb-6 border-b border-gray-200">
                ...
                <div>
                  <h2 className="text-2xl font-semibold text-[#2C2A29]">Final Summary</h2>
              ...
              <div className="space-y-4">
                <p className="text-[#2C2A29] opacity-70">
                  {data ? 'Final summary information is available.' : 'No final summary has been shared yet.'}
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-[#2C2A29] opacity-70">No final summary has been shared yet.</p>
            </div>
          )}
        </main>
      </div>
