...
export default function ViewerDigitalAccountsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!parsedSession.permissions.canViewDigitalAccounts) {
        return;
      }

      const response = await fetch('/api/viewer/data?section=digitalAccounts', {
        method: 'POST',
        headers: {
  ...
  return (
    <PermissionGate permission={session.permissions.canViewDigitalAccounts || false} sectionName="Digital Accounts">
        ...
        <header className="bg-[#FCFAF7] border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-semibold text-[#2C2A29]">Digital Accounts</h1>
          ...
          {data ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
              <div className="flex items-center space-x-3 mb-6 pb-6 border-b border-gray-200">
                ...
                <div>
                  <h2 className="text-2xl font-semibold text-[#2C2A29]">Digital Accounts</h2>
                  <p className="text-sm text-[#2C2A29] opacity-60">View-only access</p>
                </div>
              </div>

              <div className="space-y-4">
                {data.accounts && Array.isArray(data.accounts) && data.accounts.length > 0 ? (
                  data.accounts.map((account: any, index: number) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-4">
                      {account.service_name && (
                        <h3 className="text-lg font-semibold text-[#2C2A29] mb-2">{account.service_name}</h3>
                      )}
                      {account.username && (
                        <p className="text-sm text-[#2C2A29] opacity-70">Username: {account.username}</p>
                      )}
                    </div>
                  ))
                ) : (
                  <p className="text-[#2C2A29] opacity-70">No digital accounts available.</p>
                )}
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-[#2C2A29] opacity-70">No digital accounts have been shared yet.</p>
            </div>
          )}
        </main>
      </div>
