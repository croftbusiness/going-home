...
export default function ViewerReleaseSettingsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!parsedSession.permissions.canViewReleaseSettings) {
        return;
      }
  ...
  return (
    <PermissionGate permission={session.permissions.canViewReleaseSettings || false} sectionName="Release Settings">
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          {data ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
              <div className="flex items-center space-x-3 mb-6 pb-6 border-b border-gray-200">
                <div className="p-3 bg-[#93B0C8] bg-opacity-10 rounded-lg">
                  <Shield className="w-6 h-6 text-[#93B0C8]" />
                </div>
                <div>
                  <h2 className="text-2xl font-semibold text-[#2C2A29]">Release Settings</h2>
              ...
              <div className="space-y-4">
                <p className="text-[#2C2A29] opacity-70">
                  {data ? 'Release settings information is available.' : 'No release settings have been shared yet.'}
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-[#2C2A29] opacity-70">No release settings have been shared yet.</p>
            </div>
          )}
        </main>
      </div>
